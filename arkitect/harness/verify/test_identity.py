...
that built it: every word of arkitect/harness/config.py's [identity] private_words, and every private
project's slug, is absent from every file the public snapshot gets (arkitect/harness/release.py).
...
from arkitect.harness import config, release
...
def shipped_text(root=HERE):
    """(relative path, text) for every shipped file that reads as text: with the projects in
       a repository of their own (arkitect/lib/workspace.py), every file the engine tracks; in one
       repository holding both, what arkitect/harness/release.py's snapshot would get."""
