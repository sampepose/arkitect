...
This is a module of its own because BOTH lib/draw/page.py (the sheet) and
lib/draw/plan.py (the plan drawn inside it) need it, and plan.py cannot reach it through
...
rooms. Every name is re-exported from lib/draw/page.py, which is where the rest of the
...
def document(canvas, titleblock=(), observers=None):
    ...
    DIFFERENT threads: that is the point of the ContextVar, and lib/verify/test_trace.py
...
# A tool that re-runs a build to record what it draws — lib/export/dxf.py for the DXF,
# lib/verify/trace.py for the golden master — has to know which sheet is being drawn and
...
def observe(o):
    """Register an observer. Pass an INSTANCE — a class registers unbound methods,
       ...
       inherits; that is what lib/verify/trace.py and lib/export/dxf.py do at import."""
