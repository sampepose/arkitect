...
They live here, not beside the hooks, because lib/verify/run_tests.py skips every
...
class GuardBashTests(unittest.TestCase):
    ...
    def test_git_add_all(self):
        for c in ('git add -A', 'git add .', 'git add --all', 'git add -u', 'git add -Av',
            ...
            self.refuses(c)
        self.allows('git add lib/verify/gate.py lib/verify/test_gate.py')
    ...
    def test_words_inside_a_message_are_not_commands(self):
        ...
        self.allows("git commit -m 'python3 lib/export/dxf.py >/dev/null 2>&1 && echo x'")

    def test_test_runners_that_pass_on_nothing(self):
        for c in ('python3 -m pytest lib', 'pytest -k grading',
                  "python3 -m unittest discover -s lib/verify -p 'tests_*.py'"):
            self.refuses(c)
        for c in ('python3 lib/verify/run_tests.py', 'python3 -m unittest lib.verify.test_gate'):
            self.allows(c)

    def test_an_oracles_stderr_is_never_discarded(self):
        for c in ('python3 lib/export/dxf.py >/dev/null 2>&1 && echo rebuilt',
                  'python3 lib/verify/trace.py t.txt 2>/dev/null',
                  'python3 projects/oak_42/build.py &>/dev/null'):
            self.refuses(c)
        self.allows('python3 lib/verify/gate.py > /tmp/gate.txt')      # stdout only
        self.allows('ls lib 2>/dev/null')                             # not an oracle

    def test_an_oracles_exit_status_is_never_masked(self):
        for c in ('python3 lib/verify/gate.py || true', 'python3 lib/verify/run_tests.py && echo ok'):
            self.refuses(c)
        self.allows('python3 lib/verify/gate.py; echo "exit $?"')
        self.allows('python3 lib/verify/gate.py && git add x')

    def test_a_worktree_does_not_rewrite_the_deliverables(self):
        self.refuses('python3 projects/oak_42/build.py', WORKTREE)
        self.refuses('python3 lib/export/dxf.py projects/elm_7/build.py', WORKTREE)
        self.allows('python3 lib/export/dxf.py projects/elm_7/build.py /tmp/x.dxf', WORKTREE)
        self.allows('python3 lib/verify/trace.py /tmp/t.txt projects/oak_42/build.py', WORKTREE)
        self.allows('python3 lib/verify/gate.py render --sheets A-101', WORKTREE)
    ...
    def test_trace_md5_is_written_by_accept_alone(self):
        ...
        for c in ('cat projects/oak_42/trace.md5', 'grep -rn trace.md5 lib',
                  ...
                  'python3 lib/verify/gate.py accept'):
            self.allows(c)
    ...
    def test_progress_json_is_written_by_its_tool_alone(self):
        ...
        for c in ('cat projects/oak_42/progress.json',
                  'python3 -m harness.progress set oak_42 A-101 passes',
...
class GuardWriteTests(unittest.TestCase):
    ...
    def test_the_hook_exits_2_with_its_reason_on_stderr(self):
        ...
        r = subprocess.run([sys.executable, os.path.join(HOOKS, 'guard_write.py')],
                           input=data, capture_output=True, text=True)
        self.assertEqual(r.returncode, 2)
        self.assertIn('lib.verify.gate accept', r.stderr)
...
class StopGateTests(unittest.TestCase):
    ...
    def test_red_blocks_only_an_unattended_session(self):
        red = _report(ok=False, failures=['oak_42: the build failed'])
        self.assertEqual(self.decide(['lib/x.py'], red, attended=False)[0], 'block')
        action, msg = self.decide(['lib/x.py'], red, attended=True)
    ...
    def test_three_blocks_in_a_row_let_the_turn_end(self):
        action, msg = self.decide(['lib/x.py'], _report(), blocks=3)
        self.assertEqual(action, 'advise')
        self.assertIn('3 blocks in a row', msg)

    def test_a_checkout_without_the_gate_is_left_alone(self):
        self.assertEqual(self.decide(['lib/x.py'], None)[0], 'allow')

    # The [hooks] policy (harness/config.py) overrides the defaults one key at a time.

    def test_red_block_blocks_even_an_attended_session(self):
        red = _report(ok=False, failures=['oak_42: the build failed'])
        action, msg = self.decide(['lib/x.py'], red, attended=True, rules={'red': 'block'})
        self.assertEqual(action, 'block')
        self.assertIn('the build failed', msg)

    def test_red_advise_advises_even_an_unattended_session(self):
        red = _report(ok=False, failures=['oak_42: the build failed'])
        action, msg = self.decide(['lib/x.py'], red, attended=False, rules={'red': 'advise'})
        self.assertEqual(action, 'advise')
        self.assertIn('the build failed', msg)
    ...
    def test_red_off_lets_the_turn_end(self):
        red = _report(ok=False, failures=['oak_42: the build failed'])
        for attended in (True, False):
            self.assertEqual(self.decide(['lib/x.py'], red, attended=attended,
                                         rules={'red': 'off'}), ('allow', ''))
    ...
    def test_green_uncommitted_off_lets_the_turn_end(self):
        self.assertEqual(self.decide(['lib/x.py'], _report(),
                                     rules={'green_uncommitted': 'off'}), ('allow', ''))

    def test_one_rule_leaves_the_other_at_its_default(self):
        red = _report(ok=False, failures=['oak_42: the build failed'])
        rules = {'green_uncommitted': 'off'}
        self.assertEqual(self.decide(['lib/x.py'], red, attended=False, rules=rules)[0], 'block')
        self.assertEqual(self.decide(['lib/x.py'], red, attended=True, rules=rules)[0], 'advise')


class SettingsTests(unittest.TestCase):
    """.claude/hooks/settings.template.json wires the hooks (python3 -m harness.hooks
    ...
    def test_an_installed_checkout_is_not_out_of_date(self):
        ...
        self.assertIn(hooks.status(HERE), ('installed', 'not installed'),
                      'python3 -m harness.hooks install')
