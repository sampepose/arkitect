...
the gate on its first day, with nothing in it but what is its own.
    ...
    python3 -m harness.scaffold projects/<slug>/intake.json

The intake must be valid and must fit -- every zoning rule it does not meet states the
relief the designer chose (`python3 -m harness.intake` first). It refuses a project that exists.

WHAT IT WRITES, and why each is the shape it is:

    build.py            DOCUMENTS from lib/buildkit.py; G-001 and C-102 from
                        codes/columbus/zoning_sheets.py; check_model() runs the zoning fit.
                        Draws nothing at import.
    src/project.py      the title block (codes/columbus.titleblock) and the output names
    ...
    verify/             isolation and ProjectTests from harness/testkit.py -- imported, so
                        the second new address copies nothing from the first
    progress.json       the feature list, harness/catalog.py expanded for this program
    CLAUDE.md           only what differs here from the root CLAUDE.md
    trace.md5           written by lib/verify/gate.py accept, the one sanctioned writer

Its three deliverables are tracked by .gitignore's suffix rules, so nothing outside the
project is written. Its test floor is verify/FLOOR, which lib/verify/run_tests.py reads. Every generated src/ definition is
under three lines or an import, so lib/verify/twins.py finds nothing to count.
"""
import datetime
import os
import sys

from harness import catalog, intake as I, progress
...
Scaffolded by harness/scaffold.py from intake.json on {date}. The set grows one feature
at a time: `python3 -m harness.progress next {slug}`. Importing this module draws nothing,
...
(progress.json). G-001 and C-102 are drawn by codes/columbus/zoning_sheets.py until a
...
TEST = '''"""{address}: the drawing against trace.md5, its text against lib/verify/sheet_text.py,
...
{program} Scaffolded from `intake.json` on {date} by `harness/scaffold.py`. The repo-level
...
```sh
python3 -m harness.progress next {slug}      # the next feature: its guards, its references
...
python3 -m harness.progress set {slug} <id> passes   # refused unless the build proves it
```

`intake.json` is the program and the ONE definition of the lot; `src/sitework.py` reads it.
Change it, then `python3 -m harness.intake projects/{slug}/intake.json`, then build.
...
def _open(d, rows):
    from codes.columbus import fit as F
    items = []
...
def main(argv):
    ...
    for w in written:
        print('  ' + w)
    print('\nNext: python3 -m lib.verify.gate; then python3 -m harness.progress next %s' % slug)
