...
one of them reaches every project, and a new project's verify/ holds nothing but its name.
    ...
    ProjectTests
        ...
        holds the drawing to trace.md5, the drawn text to lib/verify/sheet_text.py's rules,
        and progress.json to what the build proves (harness/progress.py).
"""
import hashlib
import os
import subprocess
...
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))     # the engine
...
class ProjectTests:
    ...
    def test_the_drawing_matches_the_committed_digest(self):
        """Do not update the digest to make this pass: `python3 lib/verify/gate.py` names the
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            r = subprocess.run([sys.executable, os.path.join(ROOT, 'lib', 'verify', 'trace.py'),
                                dest, os.path.join(self.PROJ, 'build.py')],
                               capture_output=True, text=True, cwd=ROOT)
            self.assertEqual(r.returncode, 0, r.stderr[-400:])
            with open(dest, 'rb') as fh:
                got = hashlib.md5(fh.read()).hexdigest()
        self.assertEqual(got, want, 'the drawing moved; see python3 lib/verify/gate.py')
