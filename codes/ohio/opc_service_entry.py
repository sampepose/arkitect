...
the bars are taken off lib/model/pipe.py's OUTSIDE diameters, because a 1-1/2 in sleeve is
...
#   pipe_od, sleeve_od     what they actually measure across, lib/model/pipe.py
...
def entry_for(b, g):
    """One building's entry, from the Ground its project already states
       (codes/ohio/opc_separation.py). The concrete under the sleeve takes the cover the
