"""Run one of the engine's hooks by name, from wherever the engine is installed.
    ...
    python3 -m harness.hook stop_gate        # reads the hook's JSON on stdin, as Claude Code sends it

A projects repository outside the engine (lib/workspace.py) wires its hooks this way, so its
committed .claude/settings.json names no path: the engine is found the way any import is,
through `python3 -m harness.engine link`. If it is not linked, Python fails with "No module
...
def main(argv):
    if len(argv) != 1 or argv[0] not in names():
        print('usage: python3 -m harness.hook {%s}' % ','.join(names()), file=sys.stderr)
        return 2
