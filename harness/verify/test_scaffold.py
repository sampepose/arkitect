"""harness/scaffold.py and harness/progress.py, end to end, on a throwaway repository: the
...
from harness.verify.fixtures import example
...
class ScaffoldTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for d in ('lib', 'codes', 'harness'):
            shutil.copytree(os.path.join(HERE, d), os.path.join(root, d), ignore=skip)
    ...
    def scaffold(cls, d):
        ...
        with open(os.path.join(proj, 'intake.json'), 'w') as fh:
            json.dump(d, fh)
        return cls.run_('-m', 'harness.scaffold', os.path.join('projects', d['slug'], 'intake.json'))
    ...
    def test_the_scaffold_passes_the_gate_on_its_first_day(self):
        ...
        r = self.run_('lib/verify/gate.py', '--json')
    ...
    def test_a_second_address_copies_nothing_from_the_first(self):
        ...
        r = self.run_('lib/verify/twins.py')
        self.assertIn('TOTAL 0', r.stdout)

    # ------------------------------------------------------------ claims are proved

    def test_set_accepts_a_proved_claim_and_refuses_an_unproved_one(self):
        r = self.run_('-m', 'harness.progress', 'set', 'example_100', 'G-001', 'passes')
        self.assertEqual(r.returncode, 0, r.stderr)                 # codes.columbus.fit ran
        r = self.run_('-m', 'harness.progress', 'set', 'example_100', 'A-101', 'drawn')
    ...
    def test_a_hand_written_false_claim_fails_verify_and_the_gate(self):
        ...
        with open(path, 'w') as fh:
            json.dump(data, fh)
        r = self.run_('-m', 'harness.progress', 'verify', 'example_100')
        self.assertEqual(r.returncode, 1)
        self.assertIn('S-102 is claimed passes', r.stdout)
        self.assertNotIn('C-102 is claimed', r.stdout)
        rep = json.loads(self.run_('lib/verify/gate.py', '--json').stdout)
    ...
    def test_add_and_drop_shape_the_list_and_keep_the_reason(self):
        r = self.run_('-m', 'harness.progress', 'add', 'example_100', 'A-604', 'STAIR DETAILS',
                      '--after', 'A-602', '--guard', 'codes.ohio.rco.site_steps')
        self.assertEqual(r.returncode, 0, r.stderr)
        r = self.run_('-m', 'harness.progress', 'add', 'example_100', 'A-605', 'X',
                      '--guard', 'codes.no_such_rule')
        self.assertEqual(r.returncode, 1)                           # a guard that can never run
        r = self.run_('-m', 'harness.progress', 'drop', 'example_100', 'A-301')
    ...
    def test_it_refuses_an_intake_outside_its_project(self):
        ...
        try:
            r = self.run_('-m', 'harness.scaffold', 'intake.json')
        finally:
            os.remove(path)
        self.assertEqual(r.returncode, 1)
