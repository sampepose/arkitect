...
green it did not earn.
    ...
    python3 lib/verify/gate.py                    # fast tier, every project, against HEAD
    python3 lib/verify/gate.py --full             # and lib/verify/run_tests.py
    python3 lib/verify/gate.py --base main        # against another commit
    python3 lib/verify/gate.py --base main --expect-unchanged    # a refactor's proof
    python3 lib/verify/gate.py --project oak_42 --json
    python3 lib/verify/gate.py accept             # write trace.md5 where the drawing moved
    python3 lib/verify/gate.py render --sheets A-101,P-601 [--clip 0,0,12,9] [--dpi 150]
    python3 lib/verify/gate.py render --moved     # what moved against HEAD, base and current
...
what moved:
    ...
    per project   the build and its trace (lib/verify/trace.py), against trace.md5
                  what the build PRINTS, against the base -- the model checks only print
                  which SHEETS moved, against the base (trace.py --by-sheet)
                  lib/verify/sheet_text.py's findings
                  lib/export/dxf.py, to a scratch file: exit status and stderr
    ...
    the repo      pyflakes over every build, src, lib and codes
                  lib/verify/twins.py's total against test_twins.CEILING
                  --full: lib/verify/run_tests.py

A drawing that moved is not a failure. A drawing that moved while trace.md5 still holds
the old digest is: review what moved, then `python3 lib/verify/gate.py accept`, the one
...
A projects repository that uses this engine from outside it (lib/workspace.py) is measured
...
# repository uses this engine from outside it (lib/workspace.py).
...
# the gate imports.
TOOLS = ('lib/verify/gate.py', 'lib/verify/trace.py', 'lib/verify/sheet_text.py', 'lib/buildscript.py',
         'lib/workspace.py')
...
# 'PER D-912' for 'DOOR D-4A' found it (lib/verify/test_gate.py).
...
def _run(cmd, cwd=WORKSPACE, engine=ROOT):
    """(ran, returncode, stdout, stderr). ran is False only when the command could not
       ...
       The process's workspace is `cwd` and its engine `engine` (lib/workspace.py)."""
    try:
...
def measure(tree, slug, out, dxf=True, text=True, claims=False, engine=None):
    ...
    jobs = {
        'trace': [PY, os.path.join(engine, 'lib', 'verify', 'trace.py'), f('trace.txt'), build,
                  '--stdout', f('stdout.txt'), '--by-sheet', f('sheets.txt'),
                  '--pdf-dir', f('pdf')],
    }
    if text:
        jobs['sheet_text'] = [PY, os.path.join(engine, 'lib', 'verify', 'gate.py'), '_text',
                              build, f('text.json')]
    if dxf:
        jobs['dxf'] = [PY, os.path.join(engine, 'lib', 'export', 'dxf.py'), build, f('floor.dxf')]
    # A project with a feature list (harness/progress.py) has every claim in it proved by
    # its build. Run as a command, so lib/ never imports harness/.
    has_list = os.path.exists(os.path.join(tree, 'projects', slug, 'progress.json'))
    if claims and has_list:
        jobs['progress'] = [PY, '-m', 'harness.progress', 'verify', slug, '--json']
...
def baseline(ref, slugs, engine_ref=None):
    """(sha, {slug: dir}) for the base commit, built once per (commit, tools) and cached.
       ...
       With the projects in a repository of their own (lib/workspace.py), the base is TWO
...
# A decision's id (decisions/, harness/decisions.py). Door marks are D-1, D-4A: never three digits.
DECISION_ID = re.compile(r'\bD-\d{3}\b')


def _decisions():
    """{status: count} over decisions/*.md, read from each front matter's status line --
       a regex, not harness/decisions.py, so lib/ imports nothing from harness/."""
...
def _review(slugs):
    """{slug: {severity: open count}} from each project's review.json (harness/review.py)."""
    out = {}
    for s in slugs:
...
def _pyflakes():
    ...
    targets += [t if not SEPARATE else os.path.join(ROOT, t)
                for t in ('lib', 'codes', 'harness', os.path.join('.claude', 'hooks'))]
...
def _twins():
    try:
        from lib.verify import twins, test_twins
...
def _tests():
    ran, code, out, err = _run([PY, os.path.join(ROOT, 'lib', 'verify', 'run_tests.py')])
        ...
        for s in slugs:
            ...
            if committed is None:
                ...
                report['failures'].append(
                    ...
                    'Review what moved, then run: python3 lib/verify/gate.py accept' % (s, names))
...
def summary(r):
    ...
    for s, c in sorted((r.get('review') or {}).items()):
        if c:
            out.append('  review %s: %s open (python3 -m harness.review next %s)' % (
    ...
    if dc:
        out.append('  decisions: %d open for the designer, %d waiting on others, %d confirmed '
                   '(python3 -m harness.decisions pending)'
...
def _text(build, dest):
    ...
    from lib.verify import sheet_text
