"""The decisions ledger (decisions/, harness/decisions.py): every record valid, everything it
...
from harness import decisions as D
from lib import workspace

ROOT = D.ROOT
# A record's refs name the workspace's files and the engine's (codes/, lib/, harness/): with
...
CITED_IN = ('lib', 'codes', 'harness', 'projects', '.claude/hooks', '.claude/skills')
...
def _ref_exists(ref):
    ...
    return any(os.path.exists(os.path.join(t, path.split('::')[0])) for t in TREES)
