---
name: next-feature
description: Advance a scaffolded project by one feature — take the next item in projects/<slug>/progress.json, draw it on the shared rules in codes/ and lib/, prove it with the gate, and mark it passes. Use when asked to continue, advance or work on a project that has a progress.json, or to draw its next sheet.
---
...
```sh
python3 -m harness.progress next <slug>
```
...
- The model goes in `projects/<slug>/src/`, the sheet in `src/sheets/`. Read the project's
  ...
  `python3 -m harness.decisions about <path>` for each file you will change: a record there
  is a call already made, and an open one may be reversed by the designer, not by you.
- **Import, never copy.** A rule that cites a section is in `codes/`; geometry is in `lib/`.
  ...
  A copied definition fails `lib/verify/test_twins.py` the moment it lands.
...
python3 -m lib.verify.gate render --moved     # LOOK at every sheet that moved
python3 -m lib.verify.gate accept             # trace.md5, once the move is meant
python3 -m harness.progress set <slug> <id> passes
```
...
`python3 -m harness.decisions new "<title>" --project <slug>`, and its id in the commit), and
