...
byte-identical in the same file of two projects, and sorts each into
    ...
    python3 lib/verify/twins.py                 every file, and a TOTAL of twin lines
    python3 lib/verify/twins.py --file roof.py  one file, every name
    python3 lib/verify/twins.py --why           with the names that hold each one back

The TOTAL may not grow: lib/verify/test_twins.py holds it to the workspace's ceiling
