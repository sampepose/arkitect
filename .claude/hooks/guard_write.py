...
it. progress.json is written by `python3 -m harness.progress`, whose `set` refuses a claim
the build does not prove.

trace.md5 is written by `python3 -m lib.verify.gate accept` alone, after someone has
...
PROGRESS = ("progress.json is written by `python3 -m harness.progress set / add / drop` and "
            "nothing else: `set` refuses a claim the build does not prove, and a hand edit would "
            "skip exactly that.")


REVIEW = ("review.json is written by `python3 -m harness.review ingest / set` and nothing else: "
...
def check(path, root):
    ...
    if os.path.basename(path) == 'trace.md5':
        return ("trace.md5 is written by `python3 -m lib.verify.gate accept` and nothing else: "
    ...
    if inside and path.lower().endswith(('.pdf', '.dxf')):
        return ("the PDFs and DXFs in the checkout are deliverables a build writes; they are "
                ...
                "`python3 -m lib.verify.gate render --sheets A-101`.")
    return None
