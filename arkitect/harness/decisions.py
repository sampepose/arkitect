...
and means nothing to a plan reviewer (arkitect/lib/verify/gate.py fails a sheet that prints one).
...
has not confirmed -- who that is comes from arkitect/harness/config.py, [designer] -- and only the
...
BY = ('designer', 'agent', 'third-party')   # designer: the designer of record, arkitect/harness/config.py
