"""Tests for lib/verify/gate.py, on throwaway repositories rather than either project.
...
copy of lib/ and one synthetic two-sheet project, commits it as the base, and runs that
...
class GateTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        shutil.copytree(os.path.join(HERE, 'lib'), os.path.join(self.root, 'lib'),
    ...
    def run_gate(self, *args):
        r = subprocess.run([sys.executable, os.path.join(self.root, 'lib', 'verify', 'gate.py')]
...
class GateTests(GateTestCase):
    ...
    def test_a_dead_dxf_exporter_fails(self):
        with open(os.path.join(self.root, 'lib', 'export', 'dxf.py'), 'w') as fh:
