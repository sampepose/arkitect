...
This runs a project's build through a recording canvas — the way lib/verify/trace.py and
lib/export/dxf.py do, so no PDF reader is needed — and keeps, per sheet, every string with
its box on the page. Then:
    ...
    python3 lib/verify/sheet_text.py projects/<name>/build.py
...
def findings(pages):
    """Every finding as the one line this tool prints for it. lib/verify/gate.py reads
...
if __name__ == '__main__':
    build = buildscript.build_arg(sys.argv[1] if len(sys.argv) > 1 else None, 'sheet_text.py',
                                  'python3 lib/verify/sheet_text.py <projects/<slug>/build.py>')
    pages = read(build)
