...
Three files, each overriding the one before, all optional:
    ...
    arkitect.toml                       the workspace's (arkitect/lib/workspace.py): a private
...
DEFAULTS = {
    'designer': {
        'name': 'the designer of record',     # who confirms a decision (arkitect/harness/decisions.py)
    ...
    'identity': {
        'private_words': [],                  # never in a shipped file (arkitect/harness/verify/test_identity.py)
    },
    'verify': {
        'twins_ceiling': 0,                   # lines two projects may share (arkitect/lib/verify/test_twins.py)
    },
}
CHOICES = {
