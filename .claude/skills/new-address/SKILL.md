...
`python3 -m lib.verify.gate`, and its feature list says what comes next. Drawing the set
...
The questions are `harness/intake.py`'s `QUESTIONS` — read them there; that list is the
...
figures. The designer of record is `python3 -m harness.config get designer.name`. Read
`python3 -m harness.config show`: `[defaults] design` holds the designer's defaults, which
...
Only Columbus is encoded (`codes/columbus/__init__.py`). Another city is a code-research
...
of `harness/verify/fixtures.py`'s EXAMPLE; the slug is `harness.intake.slug_for(address)`.
Coordinates: feet, x from the LEFT side lot line looking from the street, y from the front
lot line toward the rear.

```sh
python3 -m harness.intake projects/<slug>/intake.json
```
...
python3 -m lib.verify.gate
python3 -m lib.verify.gate render --project <slug> --sheets G-001,C-102
```

The gate must pass. Read both PNGs the render prints — a sheet can pass every assert and
still read badly. A fault in them is a fault in `codes/columbus/zoning_sheets.py`, which is
shared: fix it there and prove every existing project unmoved with
`python3 -m lib.verify.gate --base main --expect-unchanged` (every project by default; `--project <slug>` names one).

## 4. Tailor the feature list

`python3 -m harness.progress status <slug>`. The catalog is a starting list. Drop a sheet
this set does not need, with the reason (`harness.progress drop <slug> <id> --note "..."`);
add one it does (`harness.progress add <slug> <id> "<title>" --after <id> --guard <module>`),
...
`.gitignore`, `lib/verify/run_tests.py` and `trace.md5`). Then report to the designer:
...
- what the intake assumed that they have not confirmed (no survey, parcel TBD, a front line
  taken from a neighbour) — record each with `python3 -m harness.decisions new "<title>"
  --project <slug>` (an open record with its question and what moves if they reverse it), and
  cite the ids in the commit;
- the next feature: `python3 -m harness.progress next <slug>`.
