...
what comes next -- and every claim in it PROVED by running the build.
    ...
    python3 -m harness.progress status <slug>          # the list, and what is next
    python3 -m harness.progress next <slug>            # the next feature, with what it needs
    python3 -m harness.progress verify <slug> [--json] # exit 1 if any claim is false
    python3 -m harness.progress set <slug> <id> <pending|drawn|passes> [--note "..."]
    python3 -m harness.progress add <slug> <id> "<title>" [--after <id>] [--guard <module>]...
    python3 -m harness.progress drop <slug> <id> --note "why this set does not need it"
...
(`probe`), which records every sheet it binds and every module of codes/ and
lib/model/fit.py whose functions it CALLS -- not imports: a rule module imported and never
...
`verify` exits 1 and lib/verify/gate.py fails. A sheet bound but still listed pending is
...
WATCHED = (os.path.join(ENGINE, 'codes') + os.sep, os.path.join(ENGINE, 'lib', 'model') + os.sep)
...
def probe(slug, root=ROOT):
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        r = subprocess.run([sys.executable, '-m', 'harness.progress', '_probe', build, out],
...
def next_text(slug, root=ROOT):
    ...
    return '\n'.join([
        ...
        'lib/verify/test_twins.py fails on a copy):',
        '  ' + ('\n  '.join(_refs(f['id'], slug, root)) or '(none in this checkout)'),
        'Then: bind it in build.py, run `python3 lib/verify/gate.py`, look at it with',
        '`python3 lib/verify/gate.py render --sheets %s`, and mark it:' % f['id'],
        '`python3 -m harness.progress set %s %s passes`.' % (slug, f['id']),
    ])
    ...
    if cmd == 'verify':
        ...
        if '--json' in argv:
            ...
            for w in v['false']:
                print('FALSE CLAIM: ' + w)
            for b in v['behind']:
                print('bound but listed pending: %s (python3 -m harness.progress set %s %s drawn)'
                      % (b, slug, b))
