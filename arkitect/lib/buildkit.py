...
arkitect/harness/scaffold.py imports it instead, so its build.py holds only what is its own: the
title block, the model check, and the sheets in binding order.

The contract is arkitect/lib/buildscript.py's: each document is fn(output_path=None,
