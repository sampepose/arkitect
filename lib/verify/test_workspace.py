"""lib/workspace.py: the engine and the projects in two directories, and the gate measuring
...
class SeparateGateTests(unittest.TestCase):
    ...
    def _gate(self, *args):
        ...
        r = subprocess.run([sys.executable, os.path.join(HERE, 'lib', 'verify', 'gate.py'), '--json']
    ...
    def test_the_hooks_it_installs_name_the_engine_by_path(self):
        ...
        r = subprocess.run([sys.executable, '-m', 'harness.hooks', 'install'], cwd=HERE, env=env,
