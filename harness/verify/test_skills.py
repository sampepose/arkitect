...
class SkillTests(unittest.TestCase):
    ...
    def test_every_progress_subcommand_named_exists(self):
        with open(os.path.join(ROOT, 'harness', 'progress.py')) as fh:
            src = fh.read()
        for name, text in skills():
    ...
    def test_every_review_subcommand_and_agent_named_exists(self):
        with open(os.path.join(ROOT, 'harness', 'review.py')) as fh:
            src = fh.read()
    ...
    def test_every_gate_flag_named_exists(self):
        with open(os.path.join(ROOT, 'lib', 'verify', 'gate.py')) as fh:
            src = fh.read()
        for name, text in skills():
            for line in re.findall(r'lib/verify/gate\.py([^`\n]*)', text):
