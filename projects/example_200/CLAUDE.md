...
A permit set for 200 EXAMPLE AVE: BUILDING 1, principal, 24'-0" x 36'-0", 2 storeys: UNIT 1 (3 BR); BUILDING 2, ADU, 22'-0" x 24'-0", 1 storey: UNIT 2 (1 BR) on a 35'-0" x 120'-0" corner lot with a 20'-0" alley. Scaffolded from `intake.json` on 2026-09-22 by `harness/scaffold.py`. The repo-level
...
```sh
python3 -m harness.progress next example_200      # the next feature: its guards, its references
...
python3 -m harness.progress set example_200 <id> passes   # refused unless the build proves it
```

`intake.json` is the program and the ONE definition of the lot; `src/sitework.py` reads it.
Change it, then `python3 -m harness.intake projects/example_200/intake.json`, then build.
