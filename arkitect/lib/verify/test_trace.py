"""Tests for the tools that re-run a build: arkitect/lib/verify/trace.py and arkitect/lib/export/dxf.py.

    python3 arkitect/lib/verify/run_tests.py       # the project's test command; see that file
    python3 arkitect/lib/verify/test_trace.py     # just this file, while working on it
...
`python3 -m unittest discover -s arkitect/lib/verify -p 'test_*.py'` matched the filename,
...
# A build script in the shape arkitect/lib/buildscript.py expects: it draws nothing at import,
...
class TraceTests(HarnessTestCase):
    ...
    def test_each_document_is_traced_on_its_own_paper(self):
        ...
        from arkitect.lib.draw.page import PW, PH, ANSI_B
    ...
    def test_no_filesystem_path_reaches_the_drawing(self):
        ...
        r = self.trace(dest, self.synth('asset.py', "from arkitect.lib import assets; c.drawImage(assets.image('compass.png'), 20, 20, width=40, height=40, mask='auto')"))
...
class SidecarTests(HarnessTestCase):
    """--stdout and --by-sheet, which arkitect/lib/verify/gate.py reads. The trace is the oracle
...
class DxfExportTests(HarnessTestCase):
    ...
    def test_it_refuses_to_write_without_a_named_output(self):
        ...
        self.check("it names the explicit-path remedy too", 'arkitect/lib/export/dxf.py' in said,
                   said[-400:])
