...
draw_device() in lib/symbols/electrical.py has raised on exactly this mistake since it
...
class LegendCoverageTests(unittest.TestCase):
    """lib/symbols/electrical.py's LAYER (every device kind E-101/E-102 can draw) and
       codes/columbus/legends.py's DEVICE_KINDS (the legend row for each) used to be one
       dict, so a kind added to one was necessarily in the other. Splitting the
       description out to codes/ (this task) removed that guarantee: nothing now stops
...
class ClearanceCoverageTests(unittest.TestCase):
    """The same split happened to the clearance pair: `codes/columbus/legends.py`'s
    ...
    def test_every_drawn_clearance_kind_has_a_caption_pair(self):
        from lib.symbols.base import ClearSpace
