...
def project_plans():
    ...
    from src.building1 import LEVEL
    from src.building2 import B2U, OA_B2, F_B2, PLAN_B2
    out = [('UNIT 1 LEVEL %d' % lv,
            [m['plan'].rect(r) for r in m['rooms']], m['plan'].poly(m['openareas']),
            [m['plan'].keep(f) for f in m['furn']]) for lv, m in sorted(LEVEL.items())]
    return out+[('BUILDING 2 UNITS 2 / 3',
