...
# arkitect/lib/draw/context.py — unchanged, and re-exported here, because arkitect/lib/draw/plan.py needs
...
from arkitect.lib.draw import context as _ctx
from arkitect.lib.draw.context import current, live_canvas, _announce
...
# The plan drawn inside the frame moved to arkitect/lib/draw/plan.py — it was half of this
# module, and a plan is not the sheet it sits on. Re-exported so that every existing
# `from arkitect.lib.draw.page import PlanDraw` keeps working. plan.py takes the layer and the
# colours from context.py, not from here, so this import runs in one direction only.
from arkitect.lib.draw import plan as _plan
