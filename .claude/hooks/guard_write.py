...
it. progress.json is written by `arkitect progress`, whose `set` refuses a claim
the build does not prove.

trace.md5 is written by `arkitect gate accept` alone, after someone has
...
PROGRESS = ("progress.json is written by `arkitect progress set / add / drop` and "
...
def check(path, root):
    ...
    if os.path.basename(path) == 'trace.md5':
        return ("trace.md5 is written by `arkitect gate accept` and nothing else: "
    ...
    if inside and path.lower().endswith(('.pdf', '.dxf')):
        return ("the PDFs and DXFs in the checkout are deliverables a build writes; they are "
                ...
                "`arkitect gate render --sheets A-101`.")
    return None
