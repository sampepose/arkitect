"""What a drawing cannot be started without, as a schema the agent fills and this checks.
    ...
    python3 -m harness.intake projects/<slug>/intake.json          # validate, then the fit
    python3 -m harness.intake projects/<slug>/intake.json --json   # the same, for a tool
...
Coordinates are codes/columbus/fit.py's: feet, x across the lot from the LEFT side lot line
...
def validate(d):
    ...
    if not slug or not SLUG.match(slug) or keyword.iskeyword(slug):
        bad.append('slug %r must be a lowercase Python identifier (test discovery imports it); '
                   'harness.intake.slug_for(address) gives one' % slug)
    if d.get('jurisdiction') not in JURISDICTIONS:
        bad.append('jurisdiction %r is not encoded: %s only. A new city is a code-research '
                   'deliverable (codes/columbus/__init__.py records what that means), not a flag'
    ...
    for k in (d.get('relief') or {}):
        from codes.columbus.fit import CITE
        if k not in CITE:
...
def massing(d):
    """The intake as codes/columbus/fit.py's Massing."""
