...
It is separate from lib/draw/page.py because a plan is not the sheet it sits on.
...
The class is re-exported from lib/draw/page.py, where it used to live, so the many
sheets that say `from lib.draw.page import PlanDraw` keep working unchanged.
"""
import math
import random
...
# lib/draw/context.py owns the document being drawn: the layer the next line lands on,
...
from lib.draw.context import GREY, POCHE, LAY, _announce, current_layer
