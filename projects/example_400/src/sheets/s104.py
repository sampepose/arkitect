...
def _plans():
    ...
    top = Y1-0.75*inch
    ox = (X0+1.4*inch, X0+1.4*inch+B1_W*E+2.8*inch)      # spread, so the details under them keep 300's width
    oy1 = top-B1_D*E
    oy2 = oy1-0.97*inch-1.1*inch-B2_D*E
    for i, level in enumerate((1, 2)):
        lv = b1_level(level)
        lv.over_plan = (lambda k: lambda pp: draw_b1_stair(pp, k))(level)
        lv.overlay = (lambda lines: lambda p: (_bracing(p, lines),
                      grey_context(p, B1_W, B1_D, 'OAK AVENUE', 'COURTYARD', '396 OAK AVE', '404 OAK AVE', top_off=4.4)))(_lines('BUILDING 1', level, br=br))
        ...
        lv2.overlay = (lambda lines: lambda p: (_bracing(p, lines),
                       grey_context(p, B2_W, B2_D, 'COURTYARD', 'PARKING AND ALLEY', '396 OAK AVE', '404 OAK AVE', top_off=8.2)))(_lines('BUILDING 2', level, br=br))
    ...
    return oy2-1.05*inch, ox[1]+B1_W*E+1.45*inch
...
def _basis(x, y, width):
    ...
    rows = [('CODE', 'RCO 602.10, 2018 IRC BASE; PRESCRIPTIVE'),
            ...
            ('', 'CS-PF BESIDE UNIT 2\'S ENTRY, BLDG 2 COURTYARD WALL'),
            ...
            ('', 'W1R: BUILDING 2, LEVEL 1, ALL FOUR LINES'),
            ('ROOF CONNECTION', '602.10.8.2 %s, WALLS A AND B' % bracing_shared.roof_connection(heel_nom=br.HEEL_NOM)),
            ('HOLD-DOWNS', ('%d LB CAPACITY WHERE DRAWN' % rco_bracing.HOLD_DOWN_LB) if _hold_downs() else 'NONE REQUIRED')]
...
def _hold_downs():
    return sum(1 for ln in br.LINES for e in ln.ends if e.hold_down is not None)
...
def _notes_text():
    ...
    return [n.upper() for n in (
        ...
        'the north wall and B at the south wall, the same at both levels. The house\'s stair wall, Building 2\'s bearing '
        'wall and the partitions are not braced wall lines.',
        ...
        f'exterior gypsum, A-601, {rco_bracing.NAIL_W1R} at the same spacing: {br.w1r_text().lower()}.',
        ...
        ('5. Ends, 602.10.7: the end conditions scheduled. ' + ((f'Hold-downs, HD: {rco_bracing.HOLD_DOWN_LB} lb capacity, fastened to the '
         'panel edge stud nearest the corner and to the foundation at Level 1, to the framing below at Level 2, '
         'installed per the manufacturer.') if _hold_downs() else 'Every end is condition 1 or 4: no hold-down is required.')),
        ...
        f'plate to the roof sheathing with no vent opening: it is the eave fireblock of S-103 detail 1, A-601 {FB["EAVES"]}. '
        'Attic intake is inboard of it, S-103 note 6. Walls 1 and 2 are gable ends: sheathing continuous to the roof sheathing.',

        f'9. Portal frame, Method CS-PF, 602.10.6.4, {pf[0]} {pf[1]}, beside Unit 2\'s entry D-1 {pf_where}: a '
        ...
        '10. Uplift, 602.10.2.1: the bracing lengths rely on the uplift load path of 602.3.5; truss connectors per the truss design, 802.11.',
    )]
...
def _d1(ox, oy, slot_w):
    ...
    d.rect(door[0], 0.0, door[1], br.DOOR_HEIGHT, fill=white, lw=0.6)
    ...
    _dim_v(d, door[0]+0.6, 0.0, br.DOOR_HEIGHT, 'CLEAR OPENING HEIGHT')
...
def _d2(ox, oy, slot_w):
    ...
    # shingle-over vent inboard of the block, over the baffle, S-103 note 6.
    ...
    R = run+0.4
    d.lab(1.8, top+IN(1)+1.8*ROOF_PITCH+SH/2.0, R, top+0.55, ('ROOF SHEATHING; EDGE NAILING', 'PER TABLE 602.3(1)'))
    d.lab(SH+IN(1.0), top*0.9, R, top-0.1, ('SOLID, TOP PLATE TO ROOF SHEATHING, NO VENT',
                                            'OPENING: THE EAVE FIREBLOCK, S-103 DETAIL 1',
                                            'AND A-601 %s. INTAKE INBOARD, S-103 NOTE 6' % FB['EAVES']))
    ...
    d.lab(wx0+EXT_STUD/2.0, PL/2.0, R, -0.25, ('2x BLOCKING ON THE PLATE, NAILED PER', 'TABLE 602.3(1); TRUSS CONNECTORS', 'PER THE TRUSS DESIGN'))
...
def _ijoist(d, x, y0, depth, lw=0.5):
    """A floor truss end-on: its two flat 2x4 chords and a web between them, centered on x."""
    fw, ft = IN(3.5), IN(1.5)
    d.rect(x-fw/2.0, y0, x+fw/2.0, y0+ft, lw=lw); d.rect(x-fw/2.0, y0+depth-ft, x+fw/2.0, y0+depth, lw=lw)
    d.line(x-fw/2.0, y0+ft, x+fw/2.0, y0+depth-ft, lw=0.3); d.line(x+fw/2.0, y0+ft, x-fw/2.0, y0+depth-ft, lw=0.3)
...
def _d3(ox, oy, slot_w):
    ...
    d.lab(SH+IN(0.6), J*0.6, R, J*0.66, ('TRUSSES PERPENDICULAR: RIM ALONG', 'THE WHOLE PANEL, 602.10.8 ITEM 1'))
    d.lab(1.95+SH+EXT_STUD/2.0, J*0.35, R, J*0.2, ('TRUSSES PARALLEL: END TRUSS OVER THE', 'WALL, OR FULL-DEPTH BLOCKING AT', '16" O.C. TO THE NEXT TRUSS, ITEM 2'))
    d.lab(SH+EXT_STUD*0.8, 0.0, R, -0.5, ('RIM OR BLOCKING TO LEVEL 1 TOP PLATE:', '8d COMMON TOE NAILS AT 6" O.C., ITEM 22'))
    d.lab(SH/2.0, -0.9, R, -1.15, ('WALL SHEATHING CONTINUOUS PAST THE', 'FLOOR; %s FLOOR TRUSSES AND RIM, S-102' % inches(J)))
...
def _d4(ox, oy, slot_w):
    ...
    _hatch_band(c, d.X(0), d.Y(0), d.X(L), d.Y(H), step=4.0)
    c.setStrokeColor(black); c.setLineWidth(0.8); c.rect(d.X(0), d.Y(0), L*sc, H*sc, fill=0, stroke=1)
    d.rect(x_door, 0.0, x_leg, br.DOOR_HEIGHT, fill=white)                             # the opening
