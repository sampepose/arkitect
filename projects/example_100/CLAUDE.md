...
```sh
arkitect progress next example_100      # the next feature: its guards, its references
...
arkitect progress set example_100 <id> passes   # refused unless the build proves it
```

`intake.json` is the program and the ONE definition of the lot; `src/sitework.py` reads it.
Change it, then `arkitect intake projects/example_100/intake.json`, then build.
