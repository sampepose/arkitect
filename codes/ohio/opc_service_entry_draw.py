...
It lives here rather than in lib/draw because every label and note on it names a code
section -- OPC 305.3, OPC 305.4.1, RCO 403.1.5 -- and lib/ is code-neutral
(lib/verify/test_neutrality.py). codes/ohio/rco/roof_draw.py and bracing_draw.py are the
...
from lib.draw.kit import c, notes_block
from lib.draw.page import GREY
from codes.ohio.opc_service_entry import MIN_COVER
...
def service_entry_section(top, left, right, e, wall_t, ftg_proj, insul_t, edge_insul_run,
    ...
    d.lab(-0.75, ctr, 'L',
          ('%s" WATER SERVICE, TOP %s BELOW' % (e.service, inches(e.bury)),
    ...
    d.lab(1.9, e.bed+pipe/2.0, 'R',
          ('THE SERVICE RISES INSIDE THE WALL',
           'INTO THE %s AGGREGATE AND RUNS' % inches(gravel_t),
           'TO ITS RISER, %s' % water_sheets))
    lo = d.flush('detail 1')
    _title(left, lo-0.26*inch, 1, 'WATER SERVICE THROUGH THE THICKENED FOOTING', _name)
    return lo-0.58*inch


def service_entry_elevation(top, left, right, e, ftg_w, bar, sheet):
    ...
    d.lab(0.0, ctr, 'R', ('THE SLEEVE AND THE', 'SERVICE — DETAIL 1'))
...
def service_entry_notes(x, y, width, e, gravel_t, bar, water_sheets, layer, cols=1, located=None):
    ...
    return notes_block(x, y, width, [
        ...
        'SE3. FOOTING — THE SERVICE PASSES THROUGH THE FOOTING, NOT UNDER IT: AT THE CROSSING THE FOOTING IS THICKENED TO %s, ITS '
        ...
        % (inches(e.thick), inches(e.drop), inches(-e.ftg_bot), inches(e.sleeve_bot-e.deep_bot), fmt(e.run))
        ...
        'SE5. INSIDE — THE SERVICE RISES WITHIN THE FOUNDATION WALL AND ITS SLAB-EDGE INSULATION INTO THE %s CLEAN AGGREGATE UNDER THE '
        'SLAB, S-101 NOTE 2, AND RUNS TO ITS RISER IN ONE CONTINUOUS LENGTH WITH NO JOINT BELOW THE SLAB, %s NOTE 6.'
        % (inches(gravel_t), water_sheets),
    ...
    ], 'WATER SERVICE ENTRY NOTES', layer, 'service entry note', cols, 5.2, 7.0)
