...
# deducts it from the stud face the rest of the plan is dimensioned to. Every pan in this
# set is bounded by a tub and a vanity rather than by a wall, so no dimension here moves
# by it; it is passed so that one that lands against a wall later cannot be measured to
# the framing.
BOARD = IN(0.5)
...
def notes():
    ...
    return [
     ...
     "     ONE MANUFACTURER'S PRODUCT LINE AND ONE COLOR FOR EACH SYSTEM IN ALL FIVE UNITS. LEAVE ONE UNOPENED GALLON OF EACH SYSTEM AND COLOR WITH THE OWNER.",
