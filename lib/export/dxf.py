...
# step with their constructors. See lib/draw/page.py.
class _Watch:
    def sheet(s, sh):
...
# serves any project: python3 -m lib.export.dxf <build.py> [<out.dxf>]
BUILD = buildscript.build_arg(sys.argv[1] if len(sys.argv) > 1 else None, 'dxf.py',
                              'python3 lib/export/dxf.py <projects/<slug>/build.py> [<out.dxf>]')
...
if len(sys.argv) > 2:
    ...
    if OUT is None:
        sys.exit(
            "lib/export/dxf.py: %s defines no DXF_OUT, and no output path was given.\n"
            "Fix one of:\n"
            "  - pass the output path as the second argument:\n"
            "      python3 lib/export/dxf.py %s <out.dxf>\n"
