...
the same one.
    ...
    ENGINE      this checkout of the engine: arkitect/lib/, arkitect/codes/, arkitect/harness/, style/, the tools
...
def setting(section, key, default, workspace=None):
    """One value from the workspace's arkitect.toml, for arkitect/lib/, which may not import
       arkitect/harness/config.py. arkitect/harness/config.py reads the same file, and lists every key."""
    import tomllib
