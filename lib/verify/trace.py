...
the whole trace to a file. Two traces that compare equal mean the sheets are identical.
    ...
    python3 lib/verify/trace.py before.txt     # on the current code
    ...refactor...
    python3 lib/verify/trace.py after.txt && diff before.txt after.txt
    python3 lib/verify/trace.py t.txt build.py --stdout out.txt --by-sheet sheets.txt --pdf-dir pdfs/
...
separate check: lib/export/dxf.py records the same canvas calls through its own proxy,
...
from lib.draw import page as sheets
...
# by --by-sheet, which is how lib/verify/gate.py names the sheets a change moved; the
...
# The project to trace, always named: python3 lib/verify/trace.py out.txt <build.py>
BUILD = buildscript.build_arg(sys.argv[2] if len(sys.argv) > 2 else None, 'trace.py',
                              'python3 lib/verify/trace.py <out.txt> <projects/<slug>/build.py>')
dest = sys.argv[1]
